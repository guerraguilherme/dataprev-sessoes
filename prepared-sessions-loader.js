...
(function(){
  const FILES=['MAT-FUNC-001.json','MAT-ALG-004A.json','EST-PROB-001.json','ML-BASE-001.json','PY-BASE-001.json','BD-MOD-001.json','PT-INT-001.json','PT-GEN-001.json','PT-ORT-001.json','PT-COE-001.json','EN-TEXT-001.json','RL-PROP-001.json','RL-TV-001.json','AT-NEWS-001.json','LEG-LAI-001.json'];
  const ready=new Map();
  function mergeInto(cat){
    ...
    return {...cat,sessions:[...map.values()],contentVersion:'2026.08.08-sessoes-08'};
  }
  ...
  Promise.all(FILES.map(async file=>{const r=await fetch(`./${file}?v=20260808-6`,{cache:'no-store'});if(!r.ok)throw new Error(`${file}: HTTP ${r.status}`);return r.json()})).then(items=>{
