...
async function simLoadCatalog(){
  try{const r=await fetch(SIM_CATALOG_URL+'?v=1',{cache:'no-store'});if(!r.ok)throw new Error('catálogo indisponível');const data=await r.json();simCatalog=data||simCatalog;simLoaded=true;if(document.getElementById('catalogCard')&&!document.getElementById('homePanel')?.classList.contains('hidden'))simInjectHome()}catch(error){console.warn('Simulados:',error)}
}
...
function simInjectHome(){
  ...
  const rows=sims.map(item=>{
    ...
    return `<article class="road-session ${st==='locked'?'locked':''}${['ready','in_progress'].includes(st)?' current':''}"><div class="road-top"><div><div class="road-code">${esc(item.id)}</div><div class="road-title">${esc(item.title)}</div><div class="road-topic">70 questões · FGV realista${answered?` · ${answered}/70 respondidas`:''}</div></div>${simChip(st)}</div><div class="road-actions">${action}</div></article>`;
  }).join('');
  const block=document.createElement('section');block.className='discipline-block simulados-block';block.innerHTML=`<button class="discipline-toggle" id="simToggle"><div class="discipline-main"><div class="discipline-name">Simulados completos</div><div class="discipline-summary">${done} concluído${done===1?'':'s'} · ${ready} pronto${ready===1?'':'s'} · ${course} em curso${pending?` · ${pending} em preparação`:''}</div><div class="mini-progress"><span style="width:${sims.length?100*done/sims.length:0}%"></span></div></div><div class="discipline-right"><span class="pill">estoque alvo: 2</span><span class="discipline-chevron">⌄</span></div></button><div class="discipline-body"><div class="sim-exam-note"><b>Modelo da prova:</b> 70 questões, sendo 40 de Conhecimentos Gerais e 30 de Conhecimentos Específicos. Cada questão pode abrir apoio; se isso ocorrer antes da resposta, o registro <b>com apoio</b> é automático.</div><div class="discipline-scroll">${simLoaded?rows:'<div class="road-empty">Carregando simulados…</div>'}</div></div>`;
...
async function simOpen(id){
  ...
  try{
    const r=await fetch('./simulados/'+item.file+'?v=1',{cache:'no-store'});if(!r.ok)throw new Error('arquivo indisponível');const data=await r.json();
...
function simSupportHtml(q,rec){const s=q.support||{};return `<div class="sim-support ${rec.supportOpen?'':'hidden'}"><div class="support-box"><h3>Explorar assunto</h3>${s.concept?`<p><b>Conceito:</b> ${esc(s.concept)}</p>`:''}${s.summary?`<p>${esc(s.summary)}</p>`:''}${s.example?`<p><b>Exemplo:</b> ${esc(s.example)}</p>`:''}${s.trap?`<p><b>Pegadinha:</b> ${esc(s.trap)}</p>`:''}<p class="small">${rec.supported?'Esta questão já está registrada como respondida com apoio.':'Se você abrir este bloco antes de confirmar a resposta, o uso de apoio será registrado automaticamente.'}</p></div></div>`}
function simRender(){
  ...
  $('simuladoPanel').innerHTML=`<div class="sim-head"><div><div class="kicker">${esc(simCurrent.id)} · Questão ${simState.currentIndex+1} de 70</div><h1>${esc(q.discipline||'Simulado')}</h1><div class="small">${answered}/70 confirmadas · tempo ativo ${fmt(simState.activeMs)}${rec.supported?' · com apoio':''}</div></div><button id="simExit">Voltar à trilha</button></div><div class="progress"><div style="width:${100*answered/70}%"></div></div><div class="exercise"><p style="white-space:pre-wrap">${esc(q.prompt)}</p>${q.options.map((o,i)=>`<button class="alt ${rec.selected===i?'selected':''}" data-sim-choice="${i}">${String.fromCharCode(65+i)}. ${esc(o)}</button>`).join('')}</div><button id="simSupportBtn" class="ghost" style="width:100%;margin-top:10px">${rec.supportOpen?'Fechar apoio':'Explorar assunto / Não entendi'}</button>${simSupportHtml(q,rec)}<div class="confidence">${['baixa','media','alta'].map(v=>`<button data-sim-confidence="${v}" class="${rec.confidence===v?'active':''}">${v==='media'?'Média':v[0].toUpperCase()+v.slice(1)}</button>`).join('')}</div><div class="sim-nav"><button id="simPrev" ${simState.currentIndex===0?'disabled':''}>Anterior</button><button id="simConfirm" class="primary">${rec.confirmed?'Salvar e próxima':'Confirmar e próxima'}</button></div><div class="small" style="margin-top:10px">A correção completa fica disponível ao finalizar o simulado, preservando a experiência de prova.</div>`;
  ...
  document.querySelectorAll('[data-sim-choice]').forEach(btn=>btn.onclick=()=>{rec.selected=Number(btn.dataset.simChoice);simState.answers[q.id]=rec;simSaveState();simRender()});
  document.querySelectorAll('[data-sim-confidence]').forEach(btn=>btn.onclick=()=>{rec.confidence=btn.dataset.simConfidence;simState.answers[q.id]=rec;simSaveState();simRender()});
  $('simSupportBtn').onclick=()=>{rec.supportOpen=!rec.supportOpen;if(rec.supportOpen&&!rec.confirmed){rec.supported=true;rec.supportOpenedAt=rec.supportOpenedAt||new Date().toISOString()}simState.answers[q.id]=rec;simSaveState();simRender()};
  $('simPrev').onclick=()=>{simUpdateClock();simState.currentIndex=Math.max(0,simState.currentIndex-1);simSaveState();simRender();scrollTop()};
  $('simConfirm').onclick=()=>{if(rec.selected===undefined)return alert('Marque uma alternativa.');if(!rec.confidence)return alert('Informe sua segurança.');rec.confirmed=true;rec.answeredAt=new Date().toISOString();simState.answers[q.id]=rec;simUpdateClock();if(simState.currentIndex<69){simState.currentIndex++;simSaveState();simRender();scrollTop()}else simFinish()};
}
