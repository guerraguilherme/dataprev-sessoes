...
async function simOpen(id){
  ...
  try{
    ...
    if(!simState.completedAt){simState.timerRunning=true;simState.lastTick=Date.now();simSaveState()}
...
function simRenderResult(){
  ...
  qs.forEach(q=>{const r=answers[q.id]||{},ok=r.confirmed===true&&r.selected===q.answer;if(ok)raw++;if(r.supported)supported++;const d=q.discipline||'Outros';by[d]=by[d]||{total:0,correct:0};by[d].total++;if(ok)by[d].correct++;if(q.group==='specific'){specific++;if(ok)specificCorrect++}});
  const weightedGeneral=qs.filter(q=>q.group!=='specific').reduce((a,q)=>a+((answers[q.id]?.confirmed===true&&answers[q.id]?.selected===q.answer)?1:0),0);const weighted=weightedGeneral+specificCorrect*2.5;
  $('simuladoPanel').innerHTML=`<div class="kicker">Simulado concluído</div><h1>${esc(simCurrent.title||simCurrent.id)}</h1><div class="stats sim-result-stats"><div class="stat"><b>${raw}/70</b><span>acertos brutos</span></div><div class="stat"><b>${weighted.toFixed(1)}</b><span>pontuação ponderada</span></div><div class="stat"><b>${specificCorrect}/${specific}</b><span>específicas</span></div><div class="stat"><b>${supported}</b><span>com apoio</span></div></div><h2>Desempenho por disciplina</h2><div class="sim-breakdown">${Object.entries(by).map(([d,v])=>`<div><b>${esc(d)}</b><span>${v.correct}/${v.total}</span></div>`).join('')}</div><details class="panel" open><summary>Revisão por questão</summary><div class="sim-review-list">${qs.map((q,i)=>{const r=answers[q.id]||{},ok=r.confirmed===true&&r.selected===q.answer;return `<details class="sim-review-item"><summary>Q${i+1} · ${esc(q.discipline||'')} · ${ok?'✓':'✕'}${r.supported?' · com apoio':''}</summary><p>${esc(q.prompt)}</p><p><b>Sua resposta:</b> ${r.selected===undefined?'—':esc(q.options[r.selected])}</p><p><b>Gabarito:</b> ${esc(q.options[q.answer])}</p><p>${esc(q.explanation||'')}</p>${q.optionExplanations?.map((x,j)=>`<p><b>${String.fromCharCode(65+j)}:</b> ${esc(x)}</p>`).join('')||''}${q.support?.summary?`<details><summary>Revisar assunto</summary><p>${esc(q.support.summary)}</p>${q.support.example?`<p><b>Exemplo:</b> ${esc(q.support.example)}</p>`:''}${q.support.trap?`<p><b>Pegadinha:</b> ${esc(q.support.trap)}</p>`:''}</details>`:''}</details>`}).join('')}</div></details><div class="row"><button id="simResultHome" class="primary">Voltar à trilha</button></div>`;
