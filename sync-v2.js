...
// Usa o endpoint já existente push_session_state/session_state_meta e mantém
// apenas o checkpoint mais recente de cada sessão na fila local.
...
// Ponte canônica para sessões cujo resultado já foi registrado no controle
// pedagógico, mas cujo localStorage atual pode ter ficado em outro contexto iOS.
// Não cria respostas nem reconstrói checkpoint; apenas corrige o estado visual.
...
function dpSnapshotCurrent(reason='state_change'){
  ...
  q[state.sessionId]={
    ...
    contentVersion:catalog?.contentVersion||state.contentVersion||'',
    queuedAt:new Date().toISOString(),
    reason
  };
  dpWriteQueue(q);
  dpScheduleFlush(state.completedAt||state.phase==='complete'?1200:DP_SYNC_NORMAL_DELAY);
}

function dpScheduleFlush(delay=DP_SYNC_NORMAL_DELAY){
  if(dpSyncTimer)return;
  dpSyncTimer=setTimeout(()=>{dpSyncTimer=null;dpFlushQueue()},delay);
}

async function dpFlushQueue(){
  if(dpSyncRunning)return;
  const cfg=readConfig();
  const queue=dpReadQueue();
  ...
  try{
    for(const [id,item] of Object.entries(queue)){
      const checksum=await sha256(item.stateJson);
      try{
        // Envio fire-and-confirm: o POST não bloqueia a interface e a confirmação
        // remove o item da fila somente depois de o Sheets reconhecer o checksum.
        ...
        for(let attempt=0;attempt<5&&!confirmed;attempt++){
          ...
          try{const meta=await jsonp(cfg.endpoint,{action:'session_state_meta',token:cfg.token,device_id:cfg.deviceId,session_id:id},9000);if(meta?.found&&meta.checksum===checksum)confirmed=meta}catch{}
        }
      ...
      }catch(error){console.warn('Auto-sync não confirmado para',id,error)}
    }
  }finally{
    dpWriteQueue(remaining);dpSyncRunning=false;
    if(Object.keys(remaining).length)dpScheduleFlush(DP_SYNC_RETRY_DELAY);
  }
}

// Salvar localmente continua sendo síncrono e imediato. A fila é consequência,
// nunca pré-condição para estudar ou navegar.
const dpBaseSaveState=saveState;
saveState=function(){dpBaseSaveState();dpSnapshotCurrent('checkpoint')};

// Status compartilhado: o histórico canônico corrige apenas falsos negativos de
// conclusão. Nunca marca sessão futura como concluída por estudo em outro canal.
const dpBaseSessionLocalStatus=sessionLocalStatus;
sessionLocalStatus=function(id){
  if(DP_CANONICAL_COMPLETED[id])return'concluida';
  return dpBaseSessionLocalStatus(id);
};

// Evita abrir como sessão em curso um checkpoint local obsoleto de algo que já
// consta como concluído na base. Como não inventamos as respostas faltantes,
// exibimos o status sem fabricar um resultado local.
...
function dpSetHomeDashboard(){
  const rows=Object.values(ROADMAP).flat();
  const statuses=rows.map(r=>sessionLocalStatus(r[0]));
  const done=statuses.filter(s=>s==='concluida').length;
  const ready=statuses.filter(s=>s==='pronta').length;
  const course=statuses.filter(s=>s==='em_curso').length;
  const pending=statuses.filter(s=>s==='pendente_geracao').length;
  ...
  if(document.getElementById('conceptStat'))document.getElementById('conceptStat').textContent=`${done}/${rows.length}`;
  if(document.getElementById('immediateStat'))document.getElementById('immediateStat').textContent=ready;
  if(document.getElementById('finalStat'))document.getElementById('finalStat').textContent=course;
  if(document.getElementById('timeStat'))document.getElementById('timeStat').textContent=pending;
  if(document.getElementById('progressBar'))document.getElementById('progressBar').style.width=`${rows.length?100*done/rows.length:0}%`;
  if(document.getElementById('progressText'))document.getElementById('progressText').textContent=`Visão geral da trilha · ${done} concluídas de ${rows.length} planejadas no mapa atual`;
  if(document.getElementById('phaseBadge'))document.getElementById('phaseBadge').textContent='Início';
}
...
// O botão manual passa a também tentar esvaziar a fila contínua antes de fazer
// o checkpoint confirmado da sessão atual.
const dpBaseSyncNow=syncNow;
syncNow=async function(){await dpFlushQueue();return dpBaseSyncNow()};

window.addEventListener('online',()=>{dpUpdateSyncBadge();dpScheduleFlush(800)});
window.addEventListener('offline',dpUpdateSyncBadge);
...
setTimeout(()=>{dpUpdateSyncBadge();if(dpQueueCount())dpScheduleFlush(1500);if(!document.getElementById('homePanel')?.classList.contains('hidden'))dpSetHomeDashboard()},300);
